...
import { ArrowRight, CalendarRange, ExternalLink, Image as ImageIcon, Landmark, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function RouteDetailDrawer({ open, onOpenChange, route, movements, artworks, museumMap, onArtworkSelect }: Props) {
  ...
  const routeMovements = useMemo(() =>
    movements.filter(m => m.lender_museum_id === route.lender_museum_id && m.borrower_museum_id === route.borrower_museum_id)
  ...
  const routeArtworks = useMemo(() => {
    ...
    return route.unique_artworks.map(id => artworkMap.get(id)).filter(Boolean) as EnrichedArtwork[];
  }, [artworks, route.unique_artworks]);

  const totalPages = Math.ceil(routeArtworks.length / ARTWORKS_PER_PAGE);
  const pagedArtworks = routeArtworks.slice(artworkPage * ARTWORKS_PER_PAGE, (artworkPage + 1) * ARTWORKS_PER_PAGE);
      ...
      <SheetContent className="w-full sm:max-w-xl overflow-hidden p-0">
        <ScrollArea className="h-full">
          <div className="p-6 space-y-5">
            ...
            {/* Activity Over Time - compact redesign */}
            ...
            {/* Artworks grid - smaller cards, pagination */}
              ...
              <div className="flex items-center justify-between">
                ...
                {totalPages > 1 && (
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-6 w-6" disabled={artworkPage === 0} onClick={() => setArtworkPage(p => p - 1)}>
                      <ChevronLeft className="h-3.5 w-3.5" />
                    </Button>
                    <span className="text-[10px] text-muted-foreground tabular-nums">{artworkPage + 1}/{totalPages}</span>
                    <Button variant="ghost" size="icon" className="h-6 w-6" disabled={artworkPage >= totalPages - 1} onClick={() => setArtworkPage(p => p + 1)}>
            ...
            {/* Movement Timeline with artwork thumbnails */}
            <div className="space-y-3">
              <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
