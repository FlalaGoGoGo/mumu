    ...
    <div className="rounded-lg border bg-card overflow-hidden">
      {imageUrl && (
        <div className="aspect-[4/3] bg-muted overflow-hidden">
          <img src={imageUrl} alt={artwork.title} className="w-full h-full object-cover" loading="lazy" />
        </div>
      )}
            ...
            <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
              {movements.map(m => (
                <div key={m.movement_id} className="text-xs space-y-0.5">
                  ...
                  {m.source_url && (
                    <a href={m.source_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
